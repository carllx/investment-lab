...
from config import *
import asyncio
from query_data_aio import aio_queryOverview, aio_queryRank
...
for path in path_all_json_overviews:
    ...
    if manger_operate_year < 3.0 : # rank 需要3年才准
        continue
...
# RanK [3Y,6Y,N]
# # 管理期内一年同类 Rank  RANK_RANGES = ["1n","6y","3y"]
df = pd.read_csv(PATH_ALL_FUNDS) # "fund_list.csv"
df['代码'] = df['代码'].apply(str).apply(lambda x: x.zfill(6))
...
for rang in RANK_RANGES:
    ...
    path_rank_folder = PATH_RANK_FOLDER +rang+"/"
    Fcodes = df2["CODE"].to_list()
    asyncio.run(aio_queryRank(codes=Fcodes, output_folder=path_rank_folder,Rang=rang))
    ...
    df2 = df2.loc[(df2[key_of_rank] <0.4)] #将过滤（刷新）df2 内不符合标准的基金，也不会进行下一轮 query
